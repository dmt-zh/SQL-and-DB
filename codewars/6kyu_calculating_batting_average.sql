-- In baseball, the batting average is a simple and most common way to measure a hitter's performace. Batting average
--  is calculated by taking all the players hits and dividing it by their number of at_bats, and it is usually displayed as a 3 digit decimal (i.e. 0.300).
...
-- We want batting_average to be rounded to the nearest thousandth, since that is how baseball fans are used to seeing it. 
...
SELECT
  player_name,
  games,
  CAST(ROUND(hits * 1.0/at_bats, 3) AS text) AS batting_average
FROM yankees
WHERE at_bats >= 100
