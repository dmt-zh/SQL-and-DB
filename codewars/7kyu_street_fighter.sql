-- Each row of the table fighters records, alongside the fighter's name, whether they won (1) or lost (0),
--  as well as the type of move that ended the bout.
...
SELECT
  ...
  sum(lost) AS lost
FROM fighters AS f
INNER JOIN winning_moves AS wm ON f.move_id = wm.id
